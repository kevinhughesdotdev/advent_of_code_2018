defmodule Aoc2018.Day04 do
  ...
  def part_two(input) do
    ...
    |> Enum.map(fn {day,v} -> {day, (Enum.group_by(v, fn x -> elem(x,2) end) |> Enum.map(fn {id, v} -> {{day,id}, Enum.count(v)} end))} end)
  ...
  def build_cal(input) do
    ...
    days = log |> Enum.map(fn l -> {day, _, _, _} = l; day end)
  ...
  def parse_log("[1518-" <> <<date::bytes-size(5)>> <> " " <> <<_hour::bytes-size(2)>> <> ":" <> <<min::bytes-size(2)>> <> "] Guard #" <> guard, _) do
    [{guard, _}, {min, _}] = [guard, min] |> Enum.map(fn x -> Integer.parse(x) end)
    {date, min, guard, :awake}
  end

  def parse_log("[1518-" <> <<date::bytes-size(5)>> <> " " <> <<_hour::bytes-size(2)>> <> ":" <> <<min::bytes-size(2)>> <> "] falls asleep", guard) do
    {min, _} = Integer.parse(min)
    {date, min, guard, :asleep}
  end

  def parse_log("[1518-" <> <<date::bytes-size(5)>> <> " " <> <<_hour::bytes-size(2)>> <> ":" <> <<min::bytes-size(2)>> <> "] wakes up", guard) do
    {min, _} = Integer.parse(min)
    {date, min, guard, :awake}
  end
end
