defmodule Aoc2018.Day06 do
  ...
  def part_two(input, region \\ 10000) do
    ...
    end |> List.flatten |> Enum.count(fn x -> x < region end)
  end

  def closest_loc(map, locations, x, y) do
    cond do
      loc = Enum.find(map, fn {lx, ly, _, _} -> {lx, ly} == {x,y} end) -> loc
      true ->
        [a, b | _] = Enum.map(locations, fn loc -> distance(loc, x, y) end)
  ...
  def map_boundaries(map) do
    ...
    |> Enum.zip
    |> Enum.map(fn x -> x = Tuple.to_list(x); [Enum.min(x), Enum.max(x)] end)
  end
